...
Large or small, a typical data analysis project will involve most — if
...
tidyverse packages that you will often use are:
...
in the `scripts` directory, one down from top-level project directory.
You can check by typing `getwd()` in the R console. If your path is
anything other than `<path>/<to>/edh7916/scripts` (or whatever you’ve
named your `student_<last_name>` repo), then use the R command `setwd()`
with relative paths to get where you need to be.

For example, if I run `getwd()` and see `~/courses/edh7916`, that means
my working directory is the top-level, not `scripts`. I can get to the
right spot by typing `setwd("./scripts")` in the console: it tells R, go
down one level into (`./scripts`) and make that the working directory.
Were I to run `getwd()` afterwards, I should see
...
**Notice** that I’m not setting (*i.e.* hard coding) the working
...
get there, and then rely on relative paths afterwards.
...
(`col_double()`). For other data (or if we had told `read_csv()` how to
parse the columns), we might see other column types like
`col_integer()`, `col_character()`, or `col_logical()`.
...
**NOTE** When a function comes from a specific library, I will sometimes
signal that using `::` notation: `<package>::<function>`. Along as
...
the file (*e.g.* `library(tidyverse)`). We won’t do that. I use this
notation so it’s clear to you which R library a function comes from.
When see `readr::read_csv()`, that means the `read_csv()` function,
which is part of the {readr} library.
...
> *Figure out average differences in college degree expectations across
> census regions; ignore missing values and use the higher of student
> and parental expectations if an observation has both*.
...
answer such that everyone understands what you’ve found.
...
4.  **Summarize** the data within region to get average degree
    expectation values
...
`x1stuedexpct` and `x1paredexpct`. Some
[pseudocode](https://en.wikipedia.org/wiki/Pseudocode) for making a new
column is:
...
> #### Quick exercise
> 
> Store 1,000 random values in `x`: `x <- rnorm(1000)`. Now run `sum(x)`
> and `x %>% sum`. Do you get the same thing?
