...
explicit the good practices that we've implicitly followed.
...
will want to save your changes to the data along the way --- that is,
save updated data files at discrete points in your data analysis. This
is particularly true for large projects in which wrangling your data
is distinct from its analysis and you don't want to run each part
every time.
...
But wait! Maybe you _do_ want to use this new binned value of EFC in
multiple projects. And maybe the binned categories are standard across
the literature. Isn't that _objective_? 
...
analysis you want to run (or the table or figure you want to
make). In many cases, you're analysis data set will be the same as
your clean data set:
...
difference between _subjective_ and _analysis_ data may not be
clear. That's okay! The main point is that as a data analyst, you make
data wrangling decisions thinking about how you will go from _raw_ to
_objective_ to _subjective_ to _analysis_ data in a logical and clear
