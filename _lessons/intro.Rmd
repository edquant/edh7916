...
little fuss. There are other options, but RStudio is a great
all-around program that we will use in this course.
...
> script associated with this module and run the first line. Welcome
> to R!
...
sometimes has other meanings in R (it's how functions set argument
options), it's generally clearer to use `<-`.
...
When you store something in an object, you can simply type the
object's name into the console to see what's in it.
...
the R that comes out of the box (sometimes also called "vanilla
R"). While it's powerful in and of itself --- you can do everything
you need with base R --- most of your scripts will make use of one of
more contributed packages. These can make your data analytic life
_much_ nicer. We'll lean heavily on the
...
> #### Quick exercise 
> Load the `tidyverse` package, which you just
...
[Jenny Bryan's guide](https://happygitwithr.com/).
...
> 5. Make a change to your `hello.txt` file locally, commit and push the change
> to the repo. Check the change on GitHub.
...
*Init: 1 January 2020; Updated: `r format(Sys.Date(), format = "%d %B %Y")`*
