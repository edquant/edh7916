...
The tidyverse is shorthand for
...
you are in the top-level project directory. You can check by typing
`getwd()` in the R console. If your path is anything other than
...
means my working directory is the top, not `scripts`. I can get to the
right spot by typing `setwd("./scripts")` in the console: it tells R,
go down one level into (`./scripts`) and make that the working
directory. Were I to run `getwd()` afterwards, I should see
...
top-level project directy --- not quite right. Checking and setting
...
types like `col_integer()` and `col_character()`.
...
load the library at the top of the file (_e.g._ `library(tidyverse)`),
but we won't do that. I use this notation so it's clear to you which R
library a function comes from. When see `readr::read_csv()`, that
means the `read_csv()` function, which is part of the {readr} library.
...
the HSLS09 data: _Figure out average differences in college degree
expectations across census regions; ignore missing values and use the
higher of student and parental expectations if there are both_.
...
1. **Mutate** a new value that's the higher of student and parental
   expectations
1. **Filter** out any observations with any missing values
1. **Summarize** the data within region to get average values
1. **Arrange** in order
...
of `x1stuedexpct` and `x1paredexpct`. The general format for making a
new column is:
...
expectations is larger than the parent expectations; if true, we'll
put that value into the new variable --- if false, we'll put the
...
has to be read from the inside out. For this reason, many analyses run
