...
By control flow, I just mean the functions that help you change how
your script is read. Scripts in R are read from top to bottom unless
specific commands tell R to skip some lines or conditionally repeat a
set of commands.

Repeating commands often involves a loop. Loops have a bad reputation
in R, mostly for being slow, but they aren't *that* slow and they are
...
looks like colon construction we've seen before, `<start>:<end>`,
...
Since we know that `length(chr_sequence) == 10`, that means that
`1:length(chr_sequence)` is the same thing as saying `1:10`. It's just
another --- more flexible way --- to get the end number of our
...
also use `if()` in a `for()` loop to set a condition that changes
behavior sometimes. 
...
Moving to a more realistic example, we could make a function that
filled in missing values, a common task we've had. First, we'll
generate some fake data with missing values.
...
does the same for `miss_val`. Instead of choosing a hard-coded value
(a [magic
