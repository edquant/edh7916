...
assigning our paths to macros we can reuse below. Notice that we have
a new subdirectory in our `data` directory: `sch_test`, which has a
subdirectory called `by_school`. These fake data represent test scores
across three subjects --- math, reading, and science --- across four
schools over six years. The two files in `sch_test` directory,
`all_schools.csv` and `all_schools_wide.csv`, represent the same data,
but in different formats. We'll use these data sets to practice
appending, joining, and reshaping.
...
are simply adding similarly structured rows to an exiting data
frame. What do I mean by similarly structured? Imagine you have a data
...
_key_, that is, a variable or index common to each data frame that's
...
groups within each year added to each unit by time period row. Let's
do the second, adding within-year across school average test scores to
each school by year observation.
