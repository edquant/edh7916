...
R. For more information, see the [supplemental material on data types
and structures](./types_structures.html).
...
|ggpubr      |add_summary        |help     |Add Summary Statistics onto a ggplot.                                       |
|Hmisc       |smean.cl.normal    |help     |Compute Summary Statistics on a Vector                                      |
|httr        |guess_media        |help     |Guess the media type of a path from its extension.                          |
|httr        |parse_media        |help     |Parse a media type.                                                         |
|igraph      |time_bins.sir      |help     |SIR model on graphs                                                         |
|matrixStats |rowMedians         |help     |Calculates the median for each row (column) in a matrix                     |
|matrixStats |rowWeightedMedians |help     |Calculates the weighted medians for each row (column) in a matrix           |
|matrixStats |weightedMad        |help     |Weighted Median Absolute Deviation (MAD)                                    |
|matrixStats |weightedMedian     |help     |Weighted Median Value                                                       |
|olsrr       |reference-media    |vignette |Media                                                                       |
|posterior   |ess_quantile       |help     |Effective sample sizes for quantiles                                        |
|posterior   |fold_draws         |help     |Fold draws around their median                                              |
|posterior   |mcse_quantile      |help     |Monte Carlo standard error for quantiles                                    |
|purrr       |accumulate         |help     |Accumulate intermediate results of a vector reduction                       |
|raster      |Summary-methods    |help     |Summary methods                                                             |
|recipes     |step_medianimpute  |help     |Impute Numeric Data Using the Median                                        |
|zoo         |rollmean           |help     |Rolling Means/Maximums/Medians/Sums                                         |
|zoo         |zoo                |help     |Z's Ordered Observations                                                    |
|stats       |mad                |help     |Median Absolute Deviation                                                   |
|stats       |median             |help     |Median Value                                                                |
|stats       |medpolish          |help     |Median Polish (Robust Twoway Decomposition) of a Matrix                     |
|stats       |runmed             |help     |Running Medians - Robust Scatter Plot Smoothing                             |
|stats       |smooth             |help     |Tukey's (Running Median) Smoothing                                          |
|stats       |smoothEnds         |help     |End Points Smoothing (for Running Medians)                                  |
|survival    |Math.Surv          |help     |Methods for Surv objects                                                    |
